...
- **CDN assets are pinned to exact versions with Subresource Integrity.** `swagger-ui-dist@5.33.0` and `redoc@2.5.4` are referenced with `integrity="sha384-…"` + `crossorigin="anonymous"` (constants in `SwaggerUiController`). A floating major tag would execute whatever the CDN serves next on a page that fetches the API spec; with SRI the browser refuses a changed file. Upgrading means bumping the version and recomputing the sha384 of the exact file together (`SwaggerUiControllerTest::testCdnAssetsArePinnedWithSubresourceIntegrity` enforces the shape).
...
- **Routes register against `EzPhp\Contracts\RouterInterface`**, not the framework's concrete `Router` — the framework's `RouterServiceProvider` binds the interface to its router, so this module needs only `ez-php/contracts` + `ez-php/http` at runtime (`ez-php/framework` is not required). Without a bound `RouterInterface` (CLI, a custom dispatcher) the route is simply not registered; `SwaggerUiController` can also be routed by hand.
...
- `SwaggerUiServiceProviderTest` — constructs the framework `Router` (a `require-dev` dependency, which also proves it satisfies `RouterInterface`) bound as `RouterInterface` in a minimal fake `ContainerInterface` (no full `Application` bootstrap), mirroring `ez-php/openapi`'s `OpenApiServiceProviderTest` pattern. Covers: controller binding, `GET /docs` route registration, graceful degradation when `RouterInterface` isn't bound, and correct defaults when `ConfigInterface` isn't bound.
